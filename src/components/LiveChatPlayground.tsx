...
const LiveChatPlayground: React.FC<LiveChatPlaygroundProps> = ({ initialPrompt, autorun }) => {
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="text-sm text-muted-foreground">Streaming with AI SDK</div>
        <div className="flex items-center gap-2">
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="outline" size="sm" onClick={clearConversation} disabled={isStreaming}>
      ...
      <div ref={scrollRef} className="space-y-3 mb-4 h-[60vh] md:h-[70vh] overflow-y-auto p-2">
        ...
        {messages.map((m, idx) => (
          <div
            key={idx}
            className={`p-3 rounded-lg ${m.role === "user" ? "bg-blue-100 dark:bg-blue-900/30 ml-8" : "bg-gray-100 dark:bg-gray-800 mr-8"}`}
          >
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center justify-end">
          <Button onClick={() => void sendMessage()} disabled={isStreaming || !input.trim()} title="Send (Cmd/Ctrl+Enter)">
            {isStreaming ? (
              <>
