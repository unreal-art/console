...
test.describe("Unreal Console E2E Tests", () => {
  ...
  test("should show wallet connect button in nav bar", async ({ page }) => {
    ...
    const navButton = page.locator(
      '.fixed.top-0 button:has-text("Connect Wallet")'
    )
    await expect(navButton).toBeVisible()
  })

  test("should show wallet connect button in hero section", async ({
    page,
  }) => {
    // Verify the connect wallet button is visible in the hero section
    const heroButton = page.locator(
      '.container.mx-auto.px-6.text-center button:has-text("Connect Wallet")'
    )
    await expect(heroButton).toBeVisible()
  })

  test("should scroll to onboarding section when wallet button is clicked", async ({
    page,
  }) => {
